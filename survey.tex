...
To scale manipulation beyond isolated datasets or single-stage workflows, modern systems must connect data acquisition, simulation, refinement, and reuse into unified pipelines. This section examines the infrastructures that make such integration possible.

\subsection{Embodied Datasets}\label{sec:eai_datasets}

Embodied datasets provide the foundation for manipulation learning. Their key differences lie in the type of supervision and knowledge they offer to downstream models. We therefore organize existing datasets into three tiers according to the kind of knowledge they are designed to convey (see Fig.~\ref{fig:evolution} and Tables~\ref{tab:grasping_datasets}--\ref{tab:foundation_datasets}).
...
These three tiers form a nested hierarchy, $\mathcal{K}_1 \subset \mathcal{K}_2 \subset \mathcal{K}_3$, in which each successive tier retains the supervision of the previous one while introducing additional structure, diversity, and transfer demands. Conceptually, this progression can be summarized as $\mathcal{K}_i = \mathcal{K}_{i-1} \oplus \mathcal{D}_i$, where $\mathcal{D}_i$ denotes the dataset introduced at tier $i$ and $\oplus$ denotes knowledge augmentation. Under this view, skill-level data establish local geometric and contact grounding, task-level data expose how such primitives are composed into goal-directed behaviors, and foundation-level data extend learning to cross-embodiment and cross-domain generalization.
...
Foundation-level datasets aggregate experience across heterogeneous robot morphologies under standardized observation and action protocols (Table~\ref{tab:foundation_datasets}), supporting cross-embodiment transfer by exposing models to generalized transition dynamics across platforms. RoboNet~\cite{dasari2020robonet} initiated multi-institutional data sharing with 162k trajectories across seven robot platforms. OXE~\cite{padalkar2023open} scaled this to over one million trajectories from 22 morphologies, standardized via the RLDS protocol, enabling the first cross-embodiment foundation model training experiments. Aggregating heterogeneous data, however, introduces observation misalignment across camera configurations and sensor setups. ARIO~\cite{wang2024ario} addresses this with approximately 3 million episodes featuring synchronized multi-view timestamps, and RH-20T~\cite{fang2023rh} extends the sensory scope beyond vision by integrating force-torque and audio signals across 110k sequences for contact-rich manipulation. More recently, AgiBot World Beta~\cite{agibot2025} provides over one million trajectories ($\sim$43.8 TB) collected from 100+ humanoid robots for bimanual coordination and 6-DoF dexterous manipulation. Complementing robot-centric data, Ego-Exo4D~\cite{grauman2024egoexo4d} provides 740 hours of synchronized first- and third-person human video across 8 activity domains, offering a bridge between abundant human demonstration footage and the exocentric viewpoints typical of robotic systems.

\subsection{Data Acquisition}\label{sec:data_acquisition}

This section examines how embodied manipulation acquires the data needed for learning at scale. We focus on three routes: collecting physically grounded interaction data in the real world, constructing reusable simulation ecosystems, and extending coverage through automatic data generation.

\subsubsection{Real-world Data Gathering}\label{sec:real_acquisition}

The earliest and still most established route to real-world robot data is direct human demonstration, where the central challenge is how faithfully human intent can be mapped into robot-executable trajectories. \textbf{MIME}~\cite{sharma2018mime} provided an early large-scale benchmark of paired human--robot demonstrations, making dataset-centric imitation learning for manipulation more concrete. The cost barrier dropped sharply with the \textbf{ALOHA} line~\cite{zhao2023learning, fu2024mobile}, which showed that low-cost leader--follower hardware can support high-quality bimanual demonstrations and later extended to mobile whole-body manipulation. As the hardware diversified, cross-platform retargeting became the next friction point: \textbf{AnyTeleop}~\cite{qin2024anyteleop} reduced the engineering burden of deploying teleoperation across heterogeneous robot arms, hands, and sensing setups, while \textbf{OPEN TEACH}~\cite{iyer2024openteach} emphasized an accessible, open-source mixed-reality interface for diverse morphologies. Specialized interfaces followed for higher-dimensional coordination---\textbf{TeleMoMa}~\cite{dass2024telemoma} targeted whole-body teleoperation for mobile manipulators through a modular multi-device interface, \textbf{Tilde}~\cite{si2024tilde} introduced a kinematic-twin interface tailored to finger-level dexterous hand data collection, and \textbf{ALPHA-BiACT}~\cite{alphabiact2024} integrated bilateral position-and-force control into a low-cost dual-arm platform for contact-aware demonstration. Wearable exoskeletons further lowered the barrier to in-the-wild collection: \textbf{AirExo}~\cite{airexo2024} captures whole-arm trajectories with contact dynamics, and its successor \textbf{AirExo-2}~\cite{fang2025airexo2} adds demonstration adaptors that convert human data into pseudo-robot experience via image inpainting and differentiable-rendering calibration.
...
Once direct demonstration pipelines became practical, the next bottleneck was no longer only control fidelity but the limited scale, diversity, and geographic scope of lab-bound collection. \textbf{RoboTurk}~\cite{mandlekar2018roboturk} was among the first to explore remote crowdsourced robot demonstration, showing that large operator pools can be harnessed through latency-compensated teleoperation interfaces. \textbf{M2R}~\cite{kim2024m2r} pushed decoupling further by showing that policies can be learned from master-device demonstrations without collecting robot-side data during teaching. This institutional scaling culminated in \textbf{DROID}~\cite{khazatsky2024droid}, which coordinated standardized collection across 18 global institutions to capture in-the-wild data in 564 distinct scenes. In parallel, portable systems began to decouple the demonstration phase from the robot's physical presence entirely: \textbf{UMI}~\cite{chi2024umi} enabled in-the-wild data collection with handheld grippers and consumer cameras, while \textbf{DexCap}~\cite{wang2024dexcap} combined SLAM, electromagnetic tracking, and 3D scene observations in a wearable mocap system for dexterous hand data. \textbf{EgoMimic}~\cite{kareer2025egomimic} leveraged egocentric human activity videos as a scalable source of supervision for robot imitation learning, and alignment frameworks like \textbf{AV-ALOHA}~\cite{chuang2025avaloha} addressed the perceptual gap between human demonstrator and robot sensor viewpoints. At the minimal-hardware extreme, \textbf{RoboPocket}~\cite{fang2026robopocket} allows users to provide AR-based corrections from a smartphone, and \textbf{AoE}~\cite{yang2026aoe} pushes toward passive, always-on egocentric capture during daily activities, suggesting a future in which large volumes of human interaction video can be harvested at very low marginal cost.

More recently, data collection has increasingly been reorganized around autonomous policy execution, with human supervision appearing only when autonomy becomes unreliable or when failures must be converted into additional training signal. \textbf{RoboCat}~\cite{bousmalis2023robocat} exemplified a self-improving loop in which a generalist agent is redeployed to generate data for subsequent rounds of training. \textbf{HITL-TAMP}~\cite{mandlekar2023hitltamp} showed that autonomous planning and selective human takeover can be combined to collect long-horizon manipulation data more efficiently than full-task teleoperation. 
\textbf{AutoRT}~\cite{gdm2024autort} reframed collection as a foundation-model-driven orchestration problem, using fleets of robots to autonomously propose and execute diverse real-world tasks guided by a VLM-based safety constitution. Rather than only scaling collection volume, \textbf{Gao et al.}~\cite{gao2024efficientdatacollection} analyzed how compositional structure in manipulation tasks can inform more efficient acquisition strategies. \textbf{Mendonca et al.}~\cite{mendonca2025continually} pushed the loop toward fully autonomous real-world RL, where robots repeatedly practice, collect outcomes, and refine their policies with minimal human oversight. The methodological foundations of correction-as-data, including offline recovery synthesis and online intervention protocols, are discussed in \S\ref{sec:correction_recovery}; industrial-scale instantiations of these loops are covered in \S\ref{sec:data_factories_robotic_arms}.

\subsubsection{Simulation Data Ecosystems}\label{sec:simulation_infrastructure}

We organize simulation platforms around three ecosystems that collectively underpin the majority of recent manipulation benchmarks (Table~\ref{tab:sim_classification}): MuJoCo for analytic contact dynamics, SAPIEN for articulated object interaction via PartNet-Mobility, and NVIDIA Isaac for GPU-parallel data synthesis at scale. Specialized platforms (PyBullet, CoppeliaSim, AI2-THOR, Habitat) remain in active use for task-specific evaluation.
...
\textbf{MuJoCo}~\cite{todorov2012mujoco} employs a soft-contact model and analytic inverse dynamics solver, yielding strong numerical stability for contact-rich manipulation. Early benchmarks---Adroit~\cite{rajeswaran2017learning} for 24-DoF dexterous manipulation and Meta-World~\cite{yu2021meta} for 50 parameterized multi-task settings---established MuJoCo as the standard for state-based motor control, though their textureless geometric primitives left a significant sim-to-real visual gap. The robosuite~\cite{zhu2020robosuite} middleware and RoboTurk~\cite{mandlekar2018roboturk} standardized large-scale demonstration collection, while LIBERO~\cite{liu2023libero} introduced procedural generation across 130 tasks for lifelong learning and VLABench~\cite{zhang2024vlabench} extended this with 100 task categories and language-conditioned instructions.

In the foundation-model era, MJX\footnote{\url{https://mujoco.readthedocs.io/en/stable/mjx.html}} re-implements the engine as a JAX computation graph for massively parallel, differentiable simulation, with MuJoCo Playground~\cite{deepmind2025playground} demonstrating zero-shot sim-to-real transfer. RoboCasa~\cite{nasiriany2024robocasa} integrates generative AI to synthesize 120 kitchen scenes with over 2,500 objects, and BiGym~\cite{chernyadev2024bigym} establishes 40 tasks for mobile bi-manual manipulation.

\textbf{SAPIEN}~\cite{xiang2020sapien} bridges visual perception with physically grounded interaction through native support for analytical joint constraints and part-level mobility annotations. The PartNet-Mobility dataset~\cite{xiang2020sapien} provides over 2,300 assets with hierarchical kinematic trees and motion-limit metadata. The ManiSkill series evolved from ManiSkill 2~\cite{gu2023maniskill2}, which integrated a PBD solver to encompass fluid and soft-body tasks with over 4 million expert frames, to ManiSkill 3~\cite{tao2025maniskill3}, which transitioned to a GPU-parallelized architecture achieving throughputs exceeding 30,000 FPS.

The maturity of this ecosystem shifts the focus toward sim-to-real evaluation and automated data synthesis. SIMPLER~\cite{li2025simpler} demonstrates high Pearson correlation between simulated and real-world success rates. RoboTwin 2.0~\cite{chen2025robotwin2} automatically synthesizes over 100,000 domain-randomized expert trajectories via multimodal LLMs and simulation-in-the-loop feedback, and RoboFactory~\cite{qin2025robofactory} extends this to multi-agent manipulation with strict compositional constraints guaranteeing collision-free collaboration.

The \textbf{NVIDIA Isaac} Gym introduces a GPU-resident simulation architecture where physics stepping, observation extraction, and reward computation remain entirely within GPU memory, yielding orders-of-magnitude throughput improvements over CPU-based simulators. The ecosystem builds on NVIDIA Omniverse and OpenUSD for ingesting heterogeneous industrial assets from CAD and CG pipelines into digital twins. Benchmarks such as BEHAVIOR-1K~\cite{li2024behavior} (built on the OmniGibson/Omniverse backend) and ARNOLD~\cite{gong2023arnold} leverage this infrastructure for household and language-grounded tasks, respectively. At the framework level, Isaac Lab~\cite{mittal2023orbit} serves as a unified, modular entry point for robot learning, supporting reinforcement learning, imitation learning, and motion planning across diverse embodiments.

Massively parallel physics stepping enables exploration of high-dimensional configuration spaces at scale: \textbf{DexGraspNet}~\cite{wang2023dexgraspnetlargescaleroboticdexterous} generates 1.32 million grasps across 5,355 objects via GPU-accelerated simulation, and \textbf{FurnitureSim}~\cite{heo2023furniturebench}, built upon Isaac Gym and \textbf{Factory}~\cite{narang2022factory}, simulates screw-fastening dynamics for long-horizon precision assembly. \textbf{BiDexHands}~\cite{chen2022bidexhands} leverages fully tensorized simulation for thousands of parallel bimanual manipulation tasks.

Beyond the three core ecosystems, specialized platforms provide critical validation grounds. \textbf{PyBullet}\footnote{\url{http://pybullet.org}} and \textbf{CoppeliaSim}~\cite{rohmer2013vrep} constitute the long-standing lightweight physics engines: RLBench~\cite{james2020rlbench} (CoppeliaSim) provides 100+ precision-critical tasks, Ravens~\cite{zhang2019raven} (PyBullet) pioneered spatial pick-and-place via Transporter Networks, and CALVIN~\cite{mees2022calvin} introduced multi-step language-conditioned task execution. Colosseum~\cite{pumacay2024colosseum} and GEM-Bench~\cite{garcia2025gembench} further augment this ecosystem with out-of-distribution robustness and hierarchical generalization metrics.

For indoor navigation and semantic reasoning, \textbf{Habitat}~\cite{szot2022habitat20} draws on photorealistic 3D reconstructions, while \textbf{AI2-THOR}~\cite{kolve2022ai2thor} provides procedurally generated interactive rooms. Key benchmarks include ALFRED~\cite{shridhar2020alfred} for language-guided task completion and OpenEQA~\cite{majumdar2024openeqa} for embodied question answering, with EmbodiedBench~\cite{yang2025embodiedbench} consolidating heterogeneous tasks across multiple platforms under consistent metrics.

A remaining gap is unified multi-physics simulation. \textbf{SoftGym}~\cite{lin2021softgym} provided early baselines for deformable object manipulation, but fragmented solvers struggle with cross-material interactions. \textbf{Genesis}~\cite{genesis2024} integrates MPM, FEM, and PBD into a single framework, supporting coupling across rigid, soft, and fluid materials, though its ecosystem remains nascent compared to the three core platforms.

\subsubsection{Automatic Data Generation}\label{sec:auto_data_gen}

Simulation and synthetic generation address data scarcity along three axes: scalable \emph{environment construction}, automated \emph{demonstration synthesis}, and learned \emph{world-model rollouts}.

Scene diversity has evolved from brute-force procedural generation to semantically controlled, physically grounded synthesis. ProcTHOR~\cite{deitke2022procthor} demonstrated that algorithmic sampling of over 10,000 randomized 3D indoor environments improves OOD generalization. Holodeck~\cite{yang2024holodeck} added semantic control by using LLMs to map natural language prompts to 3D scene configurations. PhyScene~\cite{yang2024physcene} addressed physical plausibility, using constraint solving to eliminate interpenetration and levitation artifacts. GAIA~\cite{ko2026gaia} grounds generation in real imagery: given a single RGB image and a task instruction, a VLM infers context and places task-relevant objects to produce interactive environments whose learned policies transfer to the corresponding physical scenes. Closed-loop automation has since emerged through RoboGen~\cite{wang2023robogen} (LLM-driven asset retrieval, scene generation, and policy training without human intervention), while RoboCasa~\cite{nasiriany2024robocasa} and GRUtopia~\cite{wang2024grutopia} scale this to high-fidelity kitchen environments and city-level multi-agent settings, respectively.

Demonstration synthesis has progressed from spatial augmentation of existing trajectories to LLM-driven task creation and morphology-aware generation. MimicGen~\cite{mandlekar2023mimicgen} multiplies a single human demonstration into thousands of valid trajectories via rigid-body spatial transformations. GenSim~\cite{wang2024gensim} moves to task-level automation, using LLMs to generate simulation code and reward functions for novel tasks. For complex morphologies, MoMaGen~\cite{li2026momagengeneratingdemonstrationssoft} formulates demonstration generation for bimanual mobile manipulators as constrained optimization over reachability and camera visibility. A parallel line bypasses physics simulators via 3D representations: RoboSplat~\cite{robosplat2025} operates within 3D Gaussian Splatting to perform scene-level object replacement and novel viewpoint synthesis from a single real demonstration, while Tether~\cite{liang2026tether} warps demonstrations to new configurations via semantic correspondence and employs VLMs to self-curate successful trials. Recovery and edge-case generation further extend coverage: IntervenGen~\cite{hoque2024intervengen} algorithmically generates recovery trajectories from sparse human corrections, and Fail2Progress~\cite{huang2025fail2progress} uses Stein Variational Inference in parallel simulations to mine difficult-scenario data from real failure cases. Additional extensions to dexterous grasping at scale~\cite{yang2026ultradexgrasp}, human-to-robot retargeting~\cite{yang2025omniretarget}, and foundation-model-scale pipeline automation~\cite{cai2026internvla} continue to broaden the scope of autonomous synthesis.

Generative world models learn environment dynamics directly from data, bypassing the fidelity limitations of hand-crafted physics engines for soft objects, fluids, and complex visual details. Cosmos~\cite{nvidia2025cosmos} trains on large-scale video to predict future frames that capture approximate physical regularities; its successor Cosmos-Predict2.5~\cite{nvidia2025cosmospredict25} unifies Text2World, Image2World, and Video2World in a single flow-based architecture. For robot-specific interaction, IRASim~\cite{zhu2025irasim} trains a Diffusion Transformer on real robot data to generate realistic video responses to specific commands, capturing hardware-specific kinematics and contact feedback. DreMa~\cite{barcellona2025dream} uses video generation to simulate alternative future trajectories for self-supervised action selection, while DreamGen~\cite{jang2025dreamge} extends this to explicit data synthesis by generating action-conditioned neural trajectories for cross-scene policy training. WorldGym~\cite{quevedo2025worldgym} takes this further by using generative world models as RL environments, allowing robots to learn by interacting with generated video streams rather than a physics engine. Remaining challenges include cumulative physical implausibility over long horizons, addressed by 3D geometric constraints in EnerVerse~\cite{huang2025enerverse} and embodiment-aware anchoring in AnchorDream~\cite{ye2025anchordream}, and the extraction of generalized dynamics priors from internet-scale video without robot-specific data~\cite{gao2026dreamdojo}.

\subsection{Data Refinement}\label{sec:data_augmentation}

Given the cost of real-world collection, data engines augment existing demonstrations through four mechanisms: visual and viewpoint augmentation (\S\ref{sec:trajectory_redrawing}) expands the perceptual distribution, physically grounded augmentation (\S\ref{sec:dynamics_constraints}) ensures augmented trajectories remain executable under kinematic and contact constraints, correction and recovery augmentation (\S\ref{sec:correction_recovery}) synthesizes the failure-recovery data absent from expert-only corpora, and data curation and reweighting (\S\ref{sec:data_refinement}) raises the effective quality of large-scale heterogeneous corpora. 

\subsubsection{Visual and Viewpoint Augmentation}\label{sec:trajectory_redrawing}

Visual augmentation modifies the appearance of existing demonstrations---backgrounds, lighting, textures, camera viewpoints---without altering the underlying actions. GenAug~\cite{chen2023genaug} applies pretrained text-to-image models to vary environments, textures, object categories, and distractors, improving few-shot imitation generalization. ROSIE~\cite{yu2023scalingrobotlearningsemantically} extends this to scene-level semantic randomization across large real-robot datasets via text-to-image diffusion. RoVi-Aug~\cite{chen2024roviaug} adds joint robot-appearance and camera-viewpoint augmentation through image-to-image diffusion, and ROPA~\cite{chen2025ropa} introduces explicit pose conditioning to synthesize third-person RGB-D data with action labels from novel viewpoints while preserving contact constraints. At the boundary of visual and physical augmentation, Action-View Augmentation~\cite{pan2025one} jointly generates novel-view image sequences and physically feasible action trajectories from a single expert episode. VISTA~\cite{wang2024vista} performs zero-shot novel view synthesis from unseen camera viewpoints, enabling view-invariant policy learning without multi-camera collection, while RoboEngine~\cite{yuan2025roboengine} packages semantic robot segmentation, background generation, and task-aware scene synthesis into a calibration-free pipeline.

A complementary direction expands the data source beyond robot demonstrations. H2R~\cite{li2026h2r} replaces human hands in egocentric videos with rendered robot end-effectors, constructing robot-centric pretraining data that bridges the visual domain gap between human and robot manipulation. LucidSim~\cite{yu2024lucidsim} couples generative visual models with physics simulators to synthesize diverse, physically plausible first-person image sequences for RGB-based sim-to-real transfer without real-world visual data.
...
As corpora scale to millions of heterogeneous trajectories, the bottleneck shifts from quantity to quality. At the dataset level, L2D~\cite{kuhar2023l2d} learns a generalizable quality estimator via preference learning to extract smooth segments from noisy demonstrations, ILID~\cite{belkhale2023data} applies distribution correction to suppress suboptimal behaviors in offline datasets, and Re-Mix~\cite{hejna2025remix} formulates multi-robot data mixing as distributionally robust optimization to mitigate negative transfer across embodiments. Finer-grained methods operate at the transition or segment level: SCIZOR~\cite{zhang2026scizor} uses a task progress predictor to discard low-quality state-action transitions and clusters visual-action embeddings to deduplicate overrepresented patterns, while JUICER~\cite{ankile2024juicer} distills redundant data to retain only informative samples and RoboPearls~\cite{tao2025robopearls} upweights rare long-tail scenarios that would otherwise be underrepresented. A more principled line grounds curation in data valuation theory: CUPID~\cite{agia2025cupidcuratingdatarobot} uses influence functions to evaluate each trajectory's gradient contribution to policy performance, DataMIL~\cite{dass2025datamil} performs end-to-end policy-aware selection via datamodels, and Robot Data Curation with Mutual Information~\cite{hejna2025robotdatami} ranks demonstrations by their information-theoretic contribution to the dataset's state-action structure. As synthetic pipelines become increasingly automated, verification closes the loop: RoboCurate~\cite{kim2026robocurate} couples VLM-based physical attribute alignment with simulator replay to verify motion consistency of generated data before it enters the training pool.

\subsection{Data Pipeline}\label{sec:data_factories_robotic_arms}

Scalable manipulation research increasingly depends on integrated infrastructures that combine procedural generation, physical simulation, and real-world data collection into closed-loop pipelines.

\textbf{RoboTwin}~\cite{mu2024robotwin} provides a data generation framework for industrial dual-arm manipulation. Its initial iteration established a high-fidelity simulation environment for contact-rich tasks by capturing the kinematics and sensor noise of physical setups. \textbf{RoboTwin v2}~\cite{chen2025robotwin2} expands the system to support MLLM-driven automated task generation, parsing abstract assembly instructions to synthesize executable task codes and asset configurations across a wide variation of industrial scenarios.

\textbf{RoboVerse}~\cite{roboverse2024} addresses data fragmentation through a standardized infrastructure, MetaSim, which integrates heterogeneous data sources and simulation interfaces. It supplies a large-scale synthetic dataset comprising over 1,000 manipulation tasks, 5,500 object assets, and approximately 10 million interaction transitions, alongside a systematic evaluation benchmark with multiple generalization levels.

\textbf{AgiBot World Colosseo}~\cite{agibot2025} integrates physical data gathering with digital synthesis for humanoid bi-manual manipulation. It deploys a fleet of over 100 AgiBot G1 humanoids to capture real-world interaction data and uses the Genie Sim engine to augment this data via physics-compliant domain randomization, producing over 1 million trajectories across 217 tasks for training the GO-1 policy.

\textbf{NVIDIA Isaac GR00T}\footnote{\url{https://developer.nvidia.com/isaac/gr00t}} provides an end-to-end platform for humanoid robot development, spanning synthetic data generation, foundation model training, and sim-to-real deployment. The \textbf{GR00T N1} foundation model pairs a diffusion transformer for high-frequency motor action generation with a vision-language module for deliberate reasoning. Its data engine, \textbf{GR00T-Dreams}, leverages Cosmos world foundation models to synthesize neural trajectories from a single image input. The ecosystem is built atop the GPU-accelerated Isaac Lab simulation framework with RTX-based rendering and Newton rigid-body dynamics.

Beyond end-to-end pipelines, the ecosystem includes simulation platforms, production middleware, scaling analyses, and task-specific data engines. GRUTOPIA~\cite{wang2024grutopia} provides a broader embodied simulation ecosystem for multi-agent social interactions; its second iteration introduces GRScenes (100,000 interactive 3D scenes) and GRResidents (an LLM-driven NPC system with long-term memory), enabling social-norm-aware data generation in populated environments. AIRSPEED~\cite{xia2024airspeed} decouples data collection, generation, and dataset construction from specific hardware backends, providing a unified API that connects VR collection rigs, mocap systems, and simulation environments into configurable pipelines. MimicLabs~\cite{mimicLabs2024} quantifies scaling laws for manipulation data---trade-offs between quantity, diversity, and quality---to inform how pipelines allocate computational budgets. LeRobot~\cite{lerobot2024} standardizes the sharing of real-world arm trajectories through an open-source repository, aggregating fragmented lab datasets into unified collections. DexFlyWheel~\cite{zhu2025dexflywheel} combines imitation learning with residual RL in simulation to autonomously collect dexterous contact trajectories, and RoboWheel~\cite{zhang2025robowheel} reconstructs contact-rich trajectories from monocular hand-object interaction videos and retargets them to diverse robot morphologies.

Having established the data infrastructures underlying large-scale manipulation research, we next examine how these data are converted into manipulation capability. The following section reviews the learning paradigms built on top of these pipelines, spanning high-level cognitive planning, low-level policy learning, and the integration architectures that bridge them.
...
\multirow{5}{*}{\textbf{B4}}
 ...
 & & 3D Neural Repr. & Geometric & Neural field & Motion planner & NDF~\cite{simeonov2021ndf}, GNFactor~\cite{ze2023gnfactor}, F3RM~\cite{shen2023distilled}, D3Fields~\cite{wang2024d3fields}, GraspSplats~\cite{graspsplats2025}, CoPa~\cite{copa2024} \\
...
The deviation from discrete 2D affordance was marked by \textbf{NDF}~\cite{simeonov2021ndf}, which formulates object representation as continuous SE(3)-equivariant coordinate fields, enabling manipulation based on shape similarity rather than memorizing global templates. \textbf{GNFactor}~\cite{ze2023gnfactor} advances this by jointly optimizing a generalizable neural feature field and a Perceiver Transformer over a shared deep 3D voxel representation, unifying scene reconstruction with decision-making for multi-task real-robot learning. To further augment this geometric substrate with open-vocabulary semantics, \textbf{F3RM}~\cite{shen2023distilled} embeds CLIP features into the neural field, allowing functional affordances (e.g., ``the handle'') to be localized via language prompts, and \textbf{D3Fields}~\cite{wang2024d3fields} extends this to dynamic scenes using DINO and CLIP features for zero-shot correspondence across unseen instances. Manipulation in 3D space requires explicit \textbf{Geometric Awareness}: \textbf{RoboSpatial}~\cite{robospatial2025} and \textbf{GeoManip}~\cite{geomanip2025} lift affordance to 3D point clouds, modeling geometric curvature and collision landscapes for kinematic feasibility. Addressing the latency of neural fields, \textbf{GraspSplats}~\cite{graspsplats2025} embeds grasp affordances directly into 3D Gaussian Splatting primitives for real-time differentiable interaction, while \textbf{CoPa}~\cite{copa2024} decomposes objects into task-relevant geometric parts to ground physics at the fine-grained component level.
...
Applying uniform-bit quantization to large robotic models typically causes significant degradation in manipulation benchmarks because action-critical channels require higher precision than language-generation channels. 
QVLA~\cite{xu2026qvla} addresses this via action-centric, channel-wise bit allocation guided by action-space sensitivity, while HBVLA~\cite{yan2026hbvla} pushes to 1-bit post-training quantization via policy-grounded saliency and Haar wavelet transforms. 
Orthogonally, token pruning attacks the input dimension: LightVLA~\cite{jiang2025lightvla} adaptively drops irrelevant visual background tokens, and EfficientVLA~\cite{yang2025efficientvla} introduces a training-free algorithm optimizing the KV cache on the fly. Beyond weight and token compression, action chunking in autoregressive models introduces temporal latency that compounds across the control loop. 
PD-VLA~\cite{pdvla2025} reformulates autoregressive decoding into parallel fixed-point iterations generating entire action chunks simultaneously; TinyVLA~\cite{wen2025tinyvla} distills massive models into fast, compact variants; SwiftVLA~\cite{ni2025swiftvla} intelligently routes spatiotemporal features to minimize compute overhead; 
SARA-RT~\cite{leal2024sara} reduces the quadratic attention complexity of large robotics transformers to linear cost by up-training softmax attention into self-adaptive robust attention, achieving approximately 14\% wall-clock speedup without degrading manipulation accuracy; 
and at the system level, Xiaomi-Robotics-0~\cite{cai2026xiaomirobotics0} achieves 80\,ms end-to-end latency via asynchronous execution with adaptive loss re-weighting. 
SP-VLA~\cite{li2025spvla} dynamically switches between the heavy foundation model and a lightweight action generator based on task complexity. These methods reveal a design spectrum from \emph{algorithmic} acceleration (PD-VLA, SwiftVLA) to \emph{system-level} pipelining (Xiaomi-Robotics-0), with the optimal combination likely being hardware-dependent.
