...
This subsection turns from data regimes to the mechanisms that populate them in practice. We organize the production stack around three routes: real-world collection of physically grounded interaction data, reusable simulation ecosystems, and automatic data generation that extends coverage beyond what can be directly observed.

\subsubsection{Real-world Data Gathering}\label{sec:real_acquisition}

Direct human demonstration remains the most established route to real-world robot data, and progress along this line has largely followed three recurring pressures: paired supervision at scale, lower-cost teleoperation, and broader embodiment coverage. \textbf{MIME}~\cite{sharma2018mime} provided an early large-scale benchmark of paired human--robot demonstrations, while the \textbf{ALOHA} line~\cite{zhao2023learning, fu2024mobile} sharply reduced the cost of collecting high-quality bimanual and later mobile whole-body data. Subsequent systems broadened the paradigm across heterogeneous hardware and richer embodiments: \textbf{AnyTeleop}~\cite{qin2024anyteleop} reduced cross-platform retargeting overhead, \textbf{Tilde}~\cite{si2024tilde} extended collection to finger-level dexterous manipulation, and \textbf{AirExo-2}~\cite{fang2025airexo2} supported wearable in-the-wild capture together with human-to-robot adaptation.

Once teleoperated collection became practical, the main bottleneck shifted from control fidelity to scale, diversity, and freedom from the laboratory. One response was distributed teleoperation: \textbf{RoboTurk}~\cite{mandlekar2018roboturk} showed that remote operators could be coordinated for large-scale data gathering, and \textbf{DROID}~\cite{khazatsky2024droid} turned this into a standardized multi-institution pipeline spanning 18 sites and 564 scenes. A second response decoupled demonstration from the robot's physical presence: \textbf{UMI}~\cite{chi2024umi} enabled portable in-the-wild collection with handheld grippers and consumer cameras, and \textbf{DexCap}~\cite{wang2024dexcap} extended this idea to dexterous capture through wearable motion tracking and 3D scene observation. More recent work treats everyday egocentric video as scalable supervision without explicit teleoperation, as in \textbf{EgoMimic}~\cite{kareer2025egomimic} and the always-on capture setting explored by \textbf{AoE}~\cite{yang2026aoe}.

More recently, data gathering has been reorganized around autonomous execution, with humans appearing mainly as supervisors, fallback operators, or sources of corrective signal. Early systems demonstrated autonomy-assisted collection loops: \textbf{RoboCat}~\cite{bousmalis2023robocat} used a self-improving cycle in which a generalist agent was redeployed to gather new experience, and \textbf{HITL-TAMP}~\cite{mandlekar2023hitltamp} combined autonomous planning with selective human takeover for long-horizon tasks. The next step scales this logic from single policies to system-level orchestration: \textbf{AutoRT}~\cite{gdm2024autort} used foundation-model-guided fleets to propose and execute diverse real-world tasks under safety constraints. Recent work on continual autonomous real-world learning extends the same logic by combining data collection, failure discovery, and policy improvement within one iterative process~\cite{mendonca2025continually}. The correction mechanisms that support such loops are discussed in \S\ref{sec:correction_recovery}, and their pipeline-level industrial instantiations in \S\ref{sec:data_factories_robotic_arms}.
...
\textbf{SAPIEN}~\cite{xiang2020sapien} addresses a different need: interaction with articulated objects whose kinematic structure must be represented explicitly. By coupling physics with part-level semantics through assets such as PartNet-Mobility and benchmarks such as ManiSkill, it has become a standard substrate for doors, drawers, tools, and other objects whose manipulation depends on handles, joints, and motion constraints. Recent systems such as SIMPLER place greater emphasis on sim-to-real evaluation, while newer pipelines built on SAPIEN increasingly use this semantic structure for automated trajectory generation and scalable asset reuse~\cite{li2025simpler}.
...
\multirow{4}{*}{\textbf{B1}}
 ...
 & & Reward Synthesis & Semantic & Reward functions & RL optimizer & Lang.\ to Reward~\cite{ma2023reward} \\
...
\midrule
\multirow{4}{*}{\textbf{B4}}
 & \multirow{5}{*}{$\mathcal{K}_1/\mathcal{K}_2$} & Physical Constraint & Geometric & Contact / SE(3) pose & Motion planner & ManipLLM~\cite{manipllm2024}, LLM-GROP~\cite{llmgrop2024}, A3VLM~\cite{huang2024a3vlm} \\
 & & 2D Affordance & Geometric & Heatmap / mask & Grasp planner & CLIPort~\cite{shridhar2021cliport}, PASG~\cite{zhu2025pasg}, Moka~\cite{moka2024}, UAD~\cite{tang2025uad} \\
 & & 3D Neural Fields & Geometric & Neural field / 3D repr. & Motion planner & NDF~\cite{simeonov2021ndf}, F3RM~\cite{shen2023distilled}, D3Fields~\cite{wang2024d3fields}, GraspSplats~\cite{graspsplats2025} \\
 & & Constraint Reasoning & Geometric & Value map / keypts & Trajectory opt. & VoxPoser~\cite{voxposer2023}, ReKep~\cite{rekep2024}, CoPa~\cite{copa2024} \\
\midrule
\multirow{2}{*}{\textbf{B5}}
 & \multirow{2}{*}{$\mathcal{K}_2/\mathcal{K}_3$} & Video / World Simulation & Predictive & Future frames & Inverse dynamics & UniPi~\cite{du2023unipi}, VILA~\cite{du2024vila}, UWM~\cite{zhu2025uwm}, RoboDreamer~\cite{zhou2024robodreamer}, V-JEPA2~\cite{assran2025vjepa} \\
...
With the functional decomposition of cognitive planning (B1--B5) and policy learning (P1--P3) in place, a natural question arises: how do different data regimes shape the way these components are coupled? Table~\ref{tab:data_learning_map} makes this mapping explicit, tracing how each data tier tends to favor particular combinations of planning, policy, and integration architecture.
...
RT-2~\cite{brohan2023rt2} marks the practical onset of modern robotic integration models by showing that language-conditioned action tokenization can unify semantic understanding and low-level control in an end-to-end policy.
(Type I) systems then scaled this paradigm through larger backbones and broader datasets (e.g., OpenVLA, RoboVLMs, UniVLA, NORA-1.5, ReconVLA), improving task breadth and cross-domain transfer\cite{kim2024openvla,li2026matters,bu2025univla,hung2025nora15,song2025reconvla}.
As autoregressive latency became a deployment bottleneck in contact-rich settings, the field moved toward two structural directions—time-scale decoupling (Type II) and spatially grounded state modeling (Type III)\cite{belkhale2024rth,chen2025fis,zhen20243dvla,qu2025spatialvla}; Table~\ref{tab:embodied_models_2026_v4} summarizes this transition from 2023 to 2026. We note that these three types are not mutually exclusive: a given model may exhibit characteristics of multiple categories (e.g., a world model with a hierarchical reasoning loop), and we classify each by its primary architectural contribution.
...
During continuous operation, the model needs to sustain 10\,Hz+ control loops on resource-constrained edge hardware while providing safety guarantees---the bottleneck has shifted from \emph{learning} to \emph{serving}.
